...
local function cupid_load(args)
    ...
    if use then
        setmetatable(cupid_stubs, {
            __index = love
        })
        setmetatable(wraped_love, {
            __index = cupid_stubs,
            __newindex = protector
        })
        _love = love
        love = wraped_love
        for k, v in pairs(protected_funcs) do
            cupid_stubs[v] = function(...)
                ...
                for km, vm in pairs(modules) do
                    if vm["paused"] and vm["paused"](vm, ...) == true then
                        paused = true
                    end
                end
                for km, vm in pairs(modules) do
                    if vm["pre-" .. v] and vm["pre-" .. v](vm, ...) == false then
                        return
                    end
                end
                ...
                for km, vm in pairs(modules) do
                    if vm["arg-" .. v] then
                        arg = {vm["arg-" .. v](vm, unpack(arg))}
                    end
                end

                if game_funcs[v] and not paused then
                    result = {select(1, xpcall(function()
                        return game_funcs[v](unpack(arg))
                    end, cupid_error))}
                end
                for km, vm in pairs(modules) do
                    if vm["post-" .. v] then
...
-- Commands
-----------------------------------------------------
local function cupid_print(str, color)
    print(str)
end
...
cupid_commands = {
    ...
    ["command"] = function(self, cmd)
        local xcmd = cmd
        if not (xcmd:match("end") or xcmd:match("do") or xcmd:match("do") or xcmd:match("function") or
            xcmd:match("return") or xcmd:match("=")) then
            xcmd = "return " .. xcmd
        end
...
setmetatable(cupid_commands.env, {
    __index = _G,
    __newindex = _G
})
...
for k, v in pairs(package.loaded) do
    cupid_keep_package[k] = true
end

local cupid_keep_global = {}
for k, v in pairs(_G) do
    cupid_keep_global[k] = true
end
...
local function cupid_reload(keep_globals)
    ...
    for k, v in pairs(package.loaded) do
        if not cupid_keep_package[k] then
    ...
    if not keep_globals then
        setmetatable(_G, {})
        for k, v in pairs(_G) do
            if not cupid_keep_global[k] then
                _G[k] = nil
            end
        end
    end

    if modules.error then
        modules.error.lasterror = nil
    end
...
cupid_commands:add("reload", function(...)
    return cupid_reload(...)
end)
...
mods.console = function()
    return {
        ...
        ["init"] = function(self)
            ...
            cupid_print = function(str, color)
        ...
        ["print"] = function(self, what, color)
            ...
            for i = self.lines + 1, #self.log do
                self.log[i] = nil
            end
        end
    }
...
mods.error = function()
    return {
        ...
        ["error"] = function(self, msg)
            ...
            if not self.always_ignore then
                self.lasterror = obj
            end
            return msg
        end,
        ...
        ["post-draw"] = function(self)
            ...
            retaining("Color", "Font", function()
                ...
                local msg = string.format("%s\n\n%s\n\n\n[C]ontinue, [A]lways, [R]eload, [E]xit", self.lasterror.msg,
                    self.lasterror.traceback)
                if self.font then
                    g.setFont(self.font)
                end
                g.setColor(255, 255, 255, 255)
    ...
    return {
        ...
        ["post-update"] = function(self, dt)
            ...
            if self.lastscan ~= nil then
                ...
                if now - self.lastscan < config.watcher_interval then
                    return
                end
                local changed = false
...
mods.physics = function()
    return {
        ...
        ["pre-load"] = function(self)
            ...
            setmetatable(wraped_physics, {
                __index = physics
            })
            rawset(wraped_love, "physics", wraped_physics)
        end,
        ["post-draw"] = function(self)
            if not config.physics_show then
                return
            end
...
if love.filesystem then
    cupid_font_data = love.filesystem.newFileData([[
